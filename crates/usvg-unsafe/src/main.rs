...
struct Args {
    ...
    shape_rendering: usvg_unsafe::ShapeRendering,
    text_rendering: usvg_unsafe::TextRendering,
    image_rendering: usvg_unsafe::ImageRendering,
...
fn process(args: Args) -> Result<(), String> {
    ...
    let mut fontdb = usvg_unsafe::fontdb::Database::new();
    ...
    if args.list_fonts {
        for face in fontdb.faces() {
            if let usvg_unsafe::fontdb::Source::File(ref path) = &face.source {
    ...
    let re_opt = usvg_unsafe::Options {
        resources_dir,
        dpi: args.dpi as f32,
        font_family: args
            .font_family
        ...
        default_size: usvg_unsafe::Size::from_wh(args.default_width as f32, args.default_height as f32)
            .unwrap(),
        image_href_resolver: usvg_unsafe::ImageHrefResolver::default(),
    };
    ...
    let tree = usvg_unsafe::Tree::from_data(&input_svg, &re_opt, &fontdb).map_err(|e| format!("{}", e))?;

    let xml_opt = usvg_unsafe::WriteOptions {
