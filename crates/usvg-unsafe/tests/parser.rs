...
fn clippath_with_invalid_child() {
    ...
    let fontdb = usvg_unsafe::fontdb::Database::new();
    let tree = usvg_unsafe::Tree::from_str(&svg, &usvg_unsafe::Options::default(), &fontdb).unwrap();
...
fn simplify_paths() {
    ...
    match path {
        usvg_unsafe::Node::Path(ref path) => {
...
fn size_detection_1() {
    ...
    let fontdb = usvg_unsafe::fontdb::Database::new();
    let tree = usvg_unsafe::Tree::from_str(&svg, &usvg_unsafe::Options::default(), &fontdb).unwrap();
    assert_eq!(tree.size(), usvg_unsafe::Size::from_wh(10.0, 20.0).unwrap());
}
...
fn size_detection_2() {
    ...
    let fontdb = usvg_unsafe::fontdb::Database::new();
    let tree = usvg_unsafe::Tree::from_str(&svg, &usvg_unsafe::Options::default(), &fontdb).unwrap();
    assert_eq!(tree.size(), usvg_unsafe::Size::from_wh(30.0, 40.0).unwrap());
}
...
fn size_detection_3() {
    ...
    let fontdb = usvg_unsafe::fontdb::Database::new();
    let tree = usvg_unsafe::Tree::from_str(&svg, &usvg_unsafe::Options::default(), &fontdb).unwrap();
    assert_eq!(tree.size(), usvg_unsafe::Size::from_wh(5.0, 20.0).unwrap());
}
...
fn size_detection_4() {
    ...
    let fontdb = usvg_unsafe::fontdb::Database::new();
    let tree = usvg_unsafe::Tree::from_str(&svg, &usvg_unsafe::Options::default(), &fontdb).unwrap();
    assert_eq!(tree.size(), usvg_unsafe::Size::from_wh(36.0, 36.0).unwrap());
    assert_eq!(tree.size(), usvg_unsafe::Size::from_wh(36.0, 36.0).unwrap());
}
...
fn size_detection_5() {
    ...
    let fontdb = usvg_unsafe::fontdb::Database::new();
    let tree = usvg_unsafe::Tree::from_str(&svg, &usvg_unsafe::Options::default(), &fontdb).unwrap();
    assert_eq!(tree.size(), usvg_unsafe::Size::from_wh(100.0, 100.0).unwrap());
}
...
fn invalid_size_1() {
    ...
    let fontdb = usvg_unsafe::fontdb::Database::new();
    let result = usvg_unsafe::Tree::from_str(&svg, &usvg_unsafe::Options::default(), &fontdb);
...
fn tree_is_send_and_sync() {
    ...
    ensure_send_and_sync::<usvg_unsafe::Tree>();
}
...
fn path_transform() {
    ...
    assert!(matches!(group_node, usvg_unsafe::Node::Group(_)));
    assert_eq!(
        group_node.abs_transform(),
        usvg_unsafe::Transform::from_translate(10.0, 0.0)
    );

    let group = match group_node {
        usvg_unsafe::Node::Group(ref g) => g,
        _ => unreachable!(),
    };

    let path = &group.children()[0];
    assert!(matches!(path, usvg_unsafe::Node::Path(_)));
    assert_eq!(
        path.abs_transform(),
        usvg_unsafe::Transform::from_translate(10.0, 0.0)
    );
...
fn path_transform_nested() {
    ...
    assert!(matches!(group_node1, usvg_unsafe::Node::Group(_)));
    assert_eq!(
        group_node1.abs_transform(),
        usvg_unsafe::Transform::from_translate(20.0, 0.0)
    );
    ...
    assert!(matches!(group_node2, usvg_unsafe::Node::Group(_)));
    assert_eq!(
        group_node2.abs_transform(),
        usvg_unsafe::Transform::from_translate(30.0, 0.0)
    );
...
fn path_transform_in_symbol_no_clip() {
    ...
    assert_eq!(group_node1.abs_transform(), usvg_unsafe::Transform::default());
...
fn path_transform_in_symbol_with_clip() {
    ...
    let group_node2 = &group1.children()[0];
    assert!(matches!(group_node2, usvg_unsafe::Node::Group(_)));
    assert_eq!(group_node2.abs_transform(), usvg_unsafe::Transform::default());
    ...
    assert!(matches!(group_node3, usvg_unsafe::Node::Group(_)));
    assert_eq!(
...
fn path_transform_in_svg() {
    ...
    assert_eq!(
        group_node1.abs_transform(),
        usvg_unsafe::Transform::from_translate(100.0, 150.0)
    );
