...
static GLOBAL_FONTDB: Lazy<std::sync::Mutex<usvg_unsafe::fontdb::Database>> = Lazy::new(|| {
...
fn resave_impl(name: &str, id_prefix: Option<String>, preserve_text: bool) {
    ...
    let tree = {
        ...
        let opt = usvg_unsafe::Options::default();
        usvg_unsafe::Tree::from_str(&input_svg, &opt, &fontdb).unwrap()
    };
    let mut xml_opt = usvg_unsafe::WriteOptions::default();
